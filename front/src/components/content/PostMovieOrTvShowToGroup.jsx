...
const PostMovieOrTvShowToGroup = ({ title, id, type, setVisible }) => {
    ...
    const postEvent = async () => {
        ...
        try {
            const response = await axios.get(`/movie/private/group/addToGroup/${type}/${id}/${group}`, {
            ...
            if(error.response.status === 400 && error.response.data === 'Event already exists') {
                setPostMovieStatus('Tapahtuma on jo lisätty ryhmään');
            } else {
                console.error('Virhe lisättäessä tapahtumaa ryhmään:', error);
                setPostMovieStatus('Virhe lisättäessä tapahtumaa ryhmään');
            }
        }
    }
